...
class WizardStochasticProblem:
    def __init__(self, an_input):
        ...
        if end - start > INIT_TIME_LIMIT:
            logging.critical("Timed out in constructor")
            raise TimeoutError("Constructor timeout")
        self.score = 0

    def run_round(self):
        """
        Run a round of the game until turns run out.
        """
        while self.state[TURNS_TO_GO] > 0:
            start = time.perf_counter()
            action = self.agent.act(deepcopy(self.state))
            ...
            if not self.is_action_legal(action):
                logging.critical("You returned an illegal action!")
                print(action)
                raise RuntimeError("Illegal action returned")
            self.result(action)
        self.terminate_execution()
    ...
    def is_action_legal(self, action):
        ...
        if len(action) != len(self.state[WIZARDS].keys()):
            logging.error(f"You gave {len(action)} atomic commands, but there are {len(self.state[WIZARDS])} wizards!")
            return False
        ...
        for atomic_action in action:
            if atomic_action[0] == 'move':
                ...
                if not _is_destroy_action_legal(atomic_action):
                    logging.error(f"Destroy action {atomic_action} is illegal!")
                    return False
            elif atomic_action[0] == 'wait':
                if not _is_wait_action_legal(atomic_action):
                    logging.error(f"Wait action {atomic_action} is illegal!")
    ...
    def environment_step(self):
        """
        Update the environment state randomly.
        """
        for t in self.state[HORCRUX]:
            horcrux_stats = self.state[HORCRUX][t]
            if random.random() < horcrux_stats['prob_change_location']:
                horcrux_stats['location'] = random.choice(horcrux_stats['possible_locations'])
        for death_eater in self.state[DEATH_EATERS]:
            de_stats = self.state[DEATH_EATERS][death_eater]
            index = de_stats["index"]
            if len(de_stats["path"]) == 1:
            ...
            if index == 0:
                ...
                de_stats["index"] = random.choice([index - 1, index, index + 1])
        self.state[TURNS_TO_GO] -= 1
    ...
    def reset_environment(self):
        ...
        """
        self.state[WIZARDS] = deepcopy(self.initial_state[WIZARDS])
        self.state[HORCRUX] = deepcopy(self.initial_state[HORCRUX])
        self.state[DEATH_EATERS] = deepcopy(self.initial_state[DEATH_EATERS])
        self.state[TURNS_TO_GO] -= 1
        self.score -= RESET_PENALTY
...
def main():
    print(f"IDS: {ids}")
    overall_scores = {}  # Dictionary to store average scores for each input.
    # For each input provided in inputs, perform 50 independent tests.
    for an_input in inputs:
        test_scores = []
        print("Testing input:")
        print(an_input)
        for test in range(50):
            try:
                ...
                problem.run_round()
                test_scores.append(problem.score)
            except EndOfGame:
                test_scores.append(problem.score)
        ...
        overall_scores[str(an_input)] = avg_score
        print(f"Average score for this input over 50 tests: {avg_score}")
    print("Overall average scores:")
    for inp, score in overall_scores.items():
        print(f"{inp}: {score}")
